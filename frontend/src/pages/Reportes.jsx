  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-slate-400 ml-2 flex-shrink-0">{extra || valor}</span>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Reportes Analíticos</h1>
        ...
        {/* Selector de período */}
        <div className="flex bg-slate-800 rounded-xl border border-slate-700 p-1 gap-0.5">
      ...
      {tabActiva === 'ventas' && (
        <div className="space-y-6">
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
            <h2 className="text-sm font-bold text-white mb-1">Ventas por hora del día</h2>
            <p className="text-xs text-slate-500 mb-4">Importe acumulado por franja horaria</p>
            {hayDatos ? (
          ...
          {(periodo === '7d' || periodo === '30d' || periodo === 'todo') && (
            <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
              <h2 className="text-sm font-bold text-white mb-1">Evolución diaria</h2>
              <p className="text-xs text-slate-500 mb-4">Últimos {periodo === '7d' ? 7 : 30} días</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-700">
                <h2 className="text-sm font-bold text-white">Últimas ventas</h2>
              </div>
              {ventas.length === 0 ? (
                ...
                <div className="divide-y divide-slate-700/50 max-h-64 overflow-y-auto">
                  {ventas.slice(0, 20).map(v => (
                    <div key={v.id} className="px-5 py-2.5 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-white">{formatPeso(v.total)}</p>
      ...
      {tabActiva === 'productos' && (
        <div className="space-y-6">
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
            <h2 className="text-sm font-bold text-white mb-1">🏆 Productos más vendidos</h2>
            <p className="text-xs text-slate-500 mb-5">Ordenados por unidades vendidas en el período</p>
            {topProds.length === 0 ? (
              ...
              <div className="space-y-3">
                {topProds.map((p, i) => (
                  <div key={p.nombre} className="flex items-center gap-3">
                    <span className="w-6 text-sm font-bold text-slate-500 text-right flex-shrink-0">
                      #{i + 1}
                    </span>
                    <span className="text-xl flex-shrink-0">{p.emoji}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-slate-200 truncate">{p.nombre}</span>
          ...
          {topProds.length > 0 && (() => {
            ...
            return (
              <div className="bg-slate-800/60 rounded-xl border border-slate-700 p-5">
                <h2 className="text-sm font-bold text-white mb-3">Resumen de facturación</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  ...
                  <div className="bg-slate-800 rounded-xl p-3 border border-slate-700">
                    <p className="text-xs text-slate-400">Tipos de producto</p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-700 flex items-center justify-between">
              <h2 className="text-sm font-bold text-white">⚠️ Insumos con alerta de stock</h2>
                ...
                {alertasInv.map(i => {
                  ...
                  return (
                    <div key={i.id} className="px-5 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-medium text-white truncate">{i.nombre}</span>
                        ...
                        <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${
                              est === 'agotado' || est === 'critico' ? 'bg-red-500' : 'bg-amber-500'
                            }`}
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-700">
              <h2 className="text-sm font-bold text-white">📋 Últimos movimientos de inventario</h2>
            </div>
                ...
                {movimientos.map(m => {
                  ...
                  return (
                    <div key={m.id} className="px-5 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-white truncate">{m.insumoNombre}</p>
                        <p className="text-xs text-slate-500">{m.fecha}{m.nota ? ` · "${m.nota}"` : ''}</p>
                      </div>
